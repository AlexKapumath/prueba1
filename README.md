...
$[r_1, r_2]= [\#(real places), \#(complex places)]$ signature of $k$, so $n=r1+2*r2$

$U =$   fundamental units of $k$. The Shintani domain corresponds to the action on $(\mathbb{R}_+})^r1\times(\mathbb{C}^{\ast})^r2$ of the group generated by $U$ (for $r1>0$ and rank of units $r=r1+r2-1>0$). The units in $U$, like all other elements of $k$, is given as a polynomial $g$ in $Q[X]$ of degree at most $n$. The associated element of $k$ is the class of $g$ in $Q[X]/(p)$
